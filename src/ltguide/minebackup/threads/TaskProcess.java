...
public class TaskProcess extends Thread {
	...
	private void process(final Process process) {
		...
		case COPY:
			...
			final boolean worldedit = plugin.config.getDestWorldEdit();
			...
			try {
				if (worldedit) {
					logAction("WorldEdit configuration", process);
					if ("copy".equals(process.getAction())) DirUtils.copyDir(sourceDir, target = new File(backupDir, format), null, filter);
					else ZipUtils.zipDir(sourceDir, target = new File(backupDir, format + ".zip"), process.getName(), plugin.config.getInt(process, "compression_level"), filter);
				} else {
